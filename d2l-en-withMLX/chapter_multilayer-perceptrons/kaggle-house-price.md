...
```{.python .input}
%%tab pytorch, mxnet, jax, tensorflow
...
def k_fold(trainer, data, k, lr):
    val_loss, models = [], []
    for i, data_fold in enumerate(k_fold_data(data, k)):
        model = d2l.LinearRegression(num_inputs=330, lr=lr) # the input shape is (1460, 330)
        model.board.yscale='log'
        if i != 0: model.board.display = False
        trainer.fit(model, data_fold)
        val_loss.append(float(model.board.data['val_loss'][-1].y))
...
```{.python .input}
%%tab pytorch, jax, mxnet, tensorflow
...
```{.python .input}
%%tab mlx
preds = [model(mx.array(data.val.values.astype(float), dtype=mx.float32))
         for model in models]
# Taking exponentiation of predictions in the logarithm scale
ensemble_preds = mx.exp(d2l.concat(preds, 1)).mean(1)
submission = pd.DataFrame({'Id':data.raw_val.Id,
                           'SalePrice':np.array(ensemble_preds)})
submission.to_csv('submission.csv', index=False)
```
