...
W = mx.random.normal(shape=(num_inputs, num_outputs), loc=0.0, scale=0.01)
b = mx.zeros(shape=num_outputs)
params = [W, b]
```
...
```{.python .input}
#@tab mlx
X = mx.random.normal(shape=(2, 5), loc=0, scale=1)
X_prob = softmax(X)
X_prob, X_prob.sum(1)
```
...
#@tab mlx
def net(X, params):
    return softmax(mx.matmul(X.reshape((-1, params[0].shape[0])), params[0]) + params[1])
```
...
#@tab mlx
def cross_entropy(y_hat, y):
    return -mx.log(y_hat[mx.arange(len(y_hat)), y])
...
```{.python .input}
#@tab mlx
evaluate_accuracy(net, test_iter, params=params)
```
...
```{.python .input}
#@tab mlx
def train_epoch_ch3(net, train_iter, loss, updater, batch_size, params): #@save
    """训练模型一个迭代周期（定义见第3章）
    Defined in :numref:`sec_softmax_scratch`"""
    # 将模型设置为训练模式
    if isinstance(net, nn.Module):
...
```{.python .input}
#@tab mlx
def train_ch3(net, train_iter, test_iter, loss, num_epochs, updater, batch_size, params): #@save
    """训练模型（定义见第3章）
    Defined in :numref:`sec_softmax_scratch`"""
    animator = Animator(xlabel='epoch', xlim=[1, num_epochs], ylim=[0.3, 0.9],
                        legend=['train loss', 'train acc', 'test acc'])
...
#@tab mlx
num_epochs = 10
train_ch3(net, train_iter, test_iter, cross_entropy, num_epochs, updater, batch_size, params)
```
...
```{.python .input}
#@tab mlx
def predict_ch3(net, test_iter, params, n=6): #@save
    """预测标签（定义见第3章）
    Defined in :numref:`sec_softmax_scratch`"""
    for X, y in test_iter:
        break
...
predict_ch3(net, test_iter, params)
```
