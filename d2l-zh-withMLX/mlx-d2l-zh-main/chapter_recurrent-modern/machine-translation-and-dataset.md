...
```{.python .input}
#@tab mlx
src_vocab = d2l.Vocab(src, min_freq=2,
...
由于机器翻译数据集由语言对组成，因此我们可以分别为源语言和目标语言构建两个词汇表。使用单词级标记化，词汇量将明显大于使用字符级标记化。为了减轻这一点，在这里我们将看起来不到两倍的未知（“”）代币视为不频繁的令牌。正如我们稍后将解释的，当使用目标序列进行训练时，解码器输出（标签标记）可以是相同的解码器输入（目标标记），移位一个标记；并且特殊的序列开头“<bos>”标记将用作预测目标序列的第一个输入标记。
...
print('source:', src.astype(mx.int32))
print('decoder input:', tgt.astype(mx.int32))
print('source len excluding pad:', src_valid_len.astype(mx.int32))
print('label:', label.astype(mx.int32))
```
我们展示了由上述 _build_arrays 方法处理的一对源序列和目标序列（以字符串格式）。
...
print('source:', data.src_vocab.to_tokens(src[0].astype(mx.int32).tolist()))
print('target:', data.tgt_vocab.to_tokens(tgt[0].astype(mx.int32).tolist()))
```
## 小结
