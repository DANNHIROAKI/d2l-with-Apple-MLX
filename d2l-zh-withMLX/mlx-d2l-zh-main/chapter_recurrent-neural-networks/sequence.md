...
def train(net, train_iter, mse_loss, epochs, lr):
    ...
    for epoch in range(epochs):
        for X, y in d2l.load_array((features[:n_train], labels[:n_train]), batch_size, is_train=True):
            loss, grad = nn.value_and_grad(net, loss_fn)(net, X, y)
            trainer.update(net, grad)
        print(f'epoch {epoch + 1}, '
              f'loss: {d2l.evaluate_loss(net, d2l.load_array((features[:n_train], labels[:n_train]), batch_size, is_train=True), mse_loss, None):f}')
